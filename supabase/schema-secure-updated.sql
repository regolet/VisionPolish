...
-- Revisions indexes
CREATE INDEX idx_revisions_order_item_id ON public.revisions(order_item_id);
CREATE INDEX idx_revisions_assigned_to ON public.revisions(assigned_to);
CREATE INDEX idx_revisions_status ON public.revisions(status);
CREATE INDEX idx_revision_images_revision_id ON public.revision_images(revision_id);
...
BEGIN
  ...
  -- Get old values for audit log (if audit_logs table exists)
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'audit_logs') THEN
